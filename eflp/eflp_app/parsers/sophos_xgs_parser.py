...
class SophosXGSParser(BaseParser):
    SYSLOG_REGEX = re.compile(
        r'^<(?P<priority>\d+)>(?P<month>\w{3})\s+(?P<day>\d{2})\s+(?P<time>\d{2}:\d{2}:\d{2})\s+(?P<host>\S+)\s+sophosxgs:\s+(?P<payload>.*)$'
    )
    ...
    def parse(self, file_path):
        ...
        with open(file_path, "r") as f:
            for line in f:
                ...
                srcip    = kv_dict.get("src_ip", "")
                dstip    = kv_dict.get("dst_ip", "")
                srcport  = self.to_int(kv_dict.get("src_port"))
                dstport  = self.to_int(kv_dict.get("dst_port"))
